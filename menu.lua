...
function Library:Keypicker(properties)
	...
	do
		...
		Library:Themify(Items.Colorpicker, "outline", "BackgroundColor3")
	...
	function Cfg.UpdateColor()
		...
		if DraggingSat then
			s = 1
				- math.clamp((offset - Items.SatValHolder.AbsolutePosition).X / Items.SatValHolder.AbsoluteSize.X, 0, 1)
			v = 1
				- math.clamp((offset - Items.SatValHolder.AbsolutePosition).Y / Items.SatValHolder.AbsoluteSize.Y, 0, 1)
	...
	function Cfg.Set(color, alpha)
		...
		if Items.Colorpicker.Visible then
			...
			if not Items.RGB.Focused then
				Items.RGB.Items.Input.Text = string.format(
					"%s, %s, %s",
	...
	Library:Connection(InputService.InputBegan, function(input)
		if
			...
			and not Library:Hovering({ Items.Colorpicker, Items.Animations.Items.DropdownElements })
...
function Library:Window(properties)
	...
	Library:Connection(RunService.RenderStepped, function()
		...
		if Tick - Cfg.Tick >= 1 then
			...
			Cfg.ChangeWatermarkTitle(
				string.format(
					'%s <font color = "%s"> / %s / %s / %sfps / %sms</font>',
					Cfg.Name,
					Library:ConvertHex(themes.preset.text_color),
					Status,
					os.date("%x / %X"),
					Cfg.Fps,
					Ping
...
function Library:Toggle(properties)
	...
	do
		...
		if Cfg.Tooltip then
			Library:Tooltip({
				Title = Cfg.Tooltip.Title,
				Text = Cfg.Tooltip.Text,
				Width = Cfg.Tooltip.Width,
				Path = Items.Title,
			})
		end
...
function Library:Tooltip(properties)
	...
	Library:Connection(InputService.InputChanged, function(input)
		if
			...
			and input.UserInputType == Enum.UserInputType.MouseMovement
			and Library:Hovering(Items.Tooltip)
		then
...
function Library:Dropdown(properties)
	...
	function Cfg.RenderOption(text)
		...
		Library:Themify(Button, "unselected", "TextColor3")
		Button.Text = text
	...
	function Cfg.RefreshOptions(options)
		...
		Items.DropdownElements.Size = dim_offset(
			Items.Outline.AbsoluteSize.X + 1,
			Cfg.Scrolling and Cfg.YSize or #Cfg.OptionInstances * themes.preset.textsize
		)
	end
...
function Library:Keybind(properties)
	...
	do
		...
		local Section = setmetatable(Cfg, Library)
		Items.Dropdown = Section:Dropdown({
			Name = "Mode",
	...
	function Cfg.SetVisible(bool)
		...
		Items.Information.Position =
			dim2(0, Items.Keybind.AbsolutePosition.X + 2, 0, Items.Keybind.AbsolutePosition.Y + 74)
	...
	Library:Connection(InputService.InputBegan, function(input, game_event)
		if input.UserInputType == Enum.UserInputType.MouseButton1 then
			if
				not (Library:Hovering(Items.Dropdown.Items.DropdownElements) or Library:Hovering(Items.Information))
...
function Library:Configs(Window, Tab)
	...
	Label:Colorpicker({
		...
		Flag = "Element Gradient 1",
	})
	Label:Colorpicker({
		Color = themes.preset.misc_2,
		Callback = function(color, alpha)
		...
		Flag = "Element Gradient 2",
	})
	Section:Slider({
		Name = "Tween Speed",
		Min = 0,
		Max = 3,
		Decimal = Library.DraggingSpeed,
		Default = 0.3,
	...
	Section:Button({
		...
		Callback = function()
			...
			Notification:NotificationButton({ Name = "Make another", Callback = function() end })
		end,
	})
	--
end
...
function Library:FadeNotification(path, is_fading) -- Horrendous dogshit code from like 500 years ago
	...
	for _, instance in path:GetDescendants() do
		...
		elseif instance:IsA("Frame") then
			Library:Tween(
				instance,
				{ BackgroundTransparency = instance.Transparency and 0 and is_fading and 1 or 0 },
				TweenInfo.new(1, Enum.EasingStyle.Exponential, Enum.EasingDirection.InOut, 0, false, 0)
			)
		end
	end
...
function Library:AddTab(properties)
	...
	do
		...
		function Math:Solve(model, viewportFrame, Camera)
			...
			for _, obj in model:GetDescendants() do
				if obj:IsA("BasePart") then
					...
					for _, corner in pairs(corners) do
						local worldPoint = cframe:PointToWorldSpace(
							Vector3.new(corner.X * size.X, corner.Y * size.Y, corner.Z * size.Z)
						)
			...
			return UDim2.fromScale(relativePos.X * 200, relativePos.Y * 200),
				UDim2.fromScale(relativeSize.X * 200, relativeSize.Y * 200)
		end
	...
	function Cfg.AddBar(props)
		...
		do -- Useless ahh
			...
			Elements.Options.Low = Section:Label({ Name = "Low" }):Colorpicker({
				...
				Flag = Config.Flag .. "_COLOR1",
				Color = props.Color1 or rgb(255, 0, 0),
			})

			Elements.Options.Medium = Section:Label({ Name = "Medium" }):Colorpicker({
				Callback = function(color)
					Elements.BarGradient.Color = rgbseq({
				...
				Flag = Config.Flag .. "_COLOR2",
				Color = props.Color1 or rgb(255, 255, 0),
			})
			...
			Elements.Options.High = Section:Label({ Name = "High" }):Colorpicker({
				...
				Flag = Config.Flag .. "_COLOR3",
				Color = props.Color1 or rgb(0, 255, 0),
			})

			Elements.Options.BarThickness = Section:Slider({
				Name = "Thickness",
				Min = 1,
				Max = 5,
				Default = 1,
			...
			Section:Toggle({
				...
				Flag = Config.Flag .. "_NUMBER",
			})

			Elements.Options.Font = Section:Dropdown({
				Name = "Font",
				Options = FontIndexes,
				Default = "ProggyClean",
				Callback = function(option)
				...
				Flag = Config.Flag .. "_FONT",
			})

			Elements.Options.TextSize = Section:Slider({
				Name = "Text Size",
				Min = 1,
				Max = 30,
				Default = 12,
				...
				Flag = Config.Flag .. "_TEXT_SIZE",
			})

			Section:Toggle({
				Name = "Enable Animations",
				Flag = Config.Flag .. "_LIST",
				Callback = function(bool)
					Options[Config.Prefix .. "_Tween"] = bool
				end,
				Flag = Config.Flag .. "_ANIMATIONS",
			})

			Elements.Options.Mode = Section:Dropdown({
				Name = "Mode",
				Options = {
					"Linear",
					"Sine",
				...
				Flag = Config.Flag .. "_MODE",
			})

			Elements.Options.Direction = Section:Dropdown({
				Name = "Direction",
				...
				Flag = Config.Flag .. "_EASING_DIRECTION",
			})

			Elements.Options.AnimationSpeed = Section:Slider({
				Name = "Speed",
				Min = 0,
				Max = 5,
				Decimal = 0.01,
				...
				Flag = Config.Flag .. "_ANIMATION_SPEED",
			})
		...
		function Config.UpdateOnParent()
			for _, newparent in { Items.Left, Items.Top, Items.Right, Items.Bottom } do
				if newparent == Elements.Type.Parent then
					...
					Elements.Bar.Size = isVertical and dim2(0, Config.Width - 2, 1, -2)
						or dim2(1, -2, 0, Config.Width - 2)
		...
		Library:Connection(InputService.InputEnded, function(input, game_event)
			if
				...
				and Elements.Information.Visible
				and not (Library:Hovering({ Elements.Information }) or Config.DetermineClosing())
			then
				Config.Tween(false)
				Config.Open = false
			end
	...
	function Cfg.AddBox(props)
		...
		do
			...
			Elements.Options.Low = Section:Label({ Name = "Low" }):Colorpicker({
				...
				Callback = function()
					...
					local Color2 = Flags[Config.Flag .. "_GRADIENT_COLOR2"]
							and Flags[Config.Flag .. "_GRADIENT_COLOR2"].Color
						or (props.Color2 or rgb(255, 255, 255))
			...
			Elements.Options.BoxFillToggle = Section:Toggle({
				Name = "Box Fill",
				Callback = function(bool)
				...
				Flag = Config.Flag .. "_BOX_FILL_ENABLED",
			})

			Elements.Options.BoxFill1 = Elements.Options.BoxFillToggle:Colorpicker({
				Name = "Top Gradient",
				Callback = function(color, alpha)
				...
				Flag = Config.Flag .. "_BOX_FILL_1",
			})

			Elements.Options.BoxFill2 = Elements.Options.BoxFillToggle:Colorpicker({
			...
			Elements.Options.Slider = Section:Slider({
				...
				Flag = Config.Flag .. "_BOX_FILL_GRADIENT_ROTATION",
			})

			for _, element in
				{
					Elements.Options.Low,
					Elements.Options.Medium,
					Elements.Options.High,
					Elements.Options.BoxFill1,
					Elements.Options.BoxFill2,
				}
			do
				Elements.Options[#Elements.Options + 1] = element.Items.Animations
			end
		...
		Elements.Text.InputEnded:Connect(function(input)
			if input.UserInputType == Enum.UserInputType.MouseButton1 then
				local Parent = Math:FindClosestFrame(
					vec2(input.Position.X, input.Position.Y - 36),
					{ Items.Box, Items.ElementHolder },
					{ "Items.Box", "Items.ElementHolder" }
				)
	...
	Items.ViewportFrame.InputEnded:Connect(function(input)
		if input.UserInputType == Enum.UserInputType.MouseButton1 then
			Dragging = false
			Cfg.VisualizedModel:SetPrimaryPartCFrame(
				CFrame.new(Vector3.new(0, Cfg.ModelOffset, -8)) * CFrame.Angles(0, math.rad(-180), 0)
			)
...
return Library, MiscOptions, Options
